...
不是 `Geant4Code_EHE` 或 `Geant4Code_CntStatOnly`。由冻结任务清单生成
20 个旋转视角的宏；Uniform/Contrast 1e9 各 200 worker、1e9 初级光子已完成并收集，
1e10 各 200 worker × 5e7 光子目前运行中。状态和路径见
[实验进度](../../docs/FOV120_EXPERIMENT_STATUS.md)。

`EventAction.cc` 在事件结束时将每块晶体累计沉积能量 (E) 加一次独立高斯噪声：
`R(E)=0.13 sqrt(511 keV/E)` 为相对 FWHM，
`σ(E)=R(E) E/2.35482`；低于零的展宽值截为零。
218 keV 相对 FWHM 19.903%、σ≈18.426 keV，能窗 196.305–239.695 keV；
440 keV 相对 FWHM 14.010%、σ≈26.177 keV，能窗 409.179–470.821 keV。
**展宽后的能量**决定 CntStat 的两个能窗，也进入两晶体 List 的能量列。
CntStat 以晶体能窗命中计数，List 按两晶体拓扑另行分类；同一事件可能同时贡献
CntStat 与 List，二者的计数和不是互不重叠的初级 γ 光子数。
FOV120 重建将 List 标为已展宽，不再随机加噪；但在 Compton 圆锥响应中仍按
同一分辨率计算角度不确定度。系统矩阵对 Gaussian 能窗接受率做解析积分，
没有在重建阶段对 CntStat 做第二次展宽。

现有 FOV120 是 218/440 γ 代理源、真空输运，不包含人体材料的衰减散射、
完整 ²²⁵Ac 衰变链或电子学死时间。产额约定 218 为 0.114、440 为 0.259。
这些限定适用于当前 FOV120 对照，不应把独立的历史 EHE/CntStat-only 项目混为一谈。
...
- 旧的 CntStat-only 示例未做串窗校正；FOV120 六路链已将 440→218 响应作为固定加性背景校正；
