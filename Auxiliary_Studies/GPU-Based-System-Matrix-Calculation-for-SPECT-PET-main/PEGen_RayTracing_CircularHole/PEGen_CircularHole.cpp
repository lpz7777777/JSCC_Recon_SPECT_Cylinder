...
int main(int argc, char* argv[])
{
	...
	fread(parameter_Detector, sizeof(float), 200000, fid1);
	fclose(fid1);
	...
	// ScatterGen needs the unwindowed matrix above to infer first-interaction
	// Compton probabilities. The second file is the detector response that is
	// physically accepted by the configured energy window.
	float minimum_acceptance = 1.0f;
	float maximum_acceptance = 0.0f;
	for (size_t idxRotation = 0; idxRotation < numRotation; ++idxRotation)
	{
		for (size_t row = 0; row < numProjectionSingle; ++row)
		{
			const float acceptance = photopeak_energy_window_acceptance(
				parameter_Physics, parameter_Detector + row * 12 + 1);
			minimum_acceptance = fminf(minimum_acceptance, acceptance);
			maximum_acceptance = fmaxf(maximum_acceptance, acceptance);
			const size_t base = (idxRotation * numProjectionSingle + row) * numImagebin;
			for (size_t col = 0; col < numImagebin; ++col)
				out[base + col] *= acceptance;
		}
	}

	char FnameWindowed[2048];
	sprintf(FnameWindowed, "PE_Windowed_SysMat_shift_%f_%f_%f_v3.sysmat", shiftFOVX, shiftFOVY, shiftFOVZ);
	FILE* fp_windowed = fopen(FnameWindowed, "wb+");
	if (fp_windowed == 0) { puts("error"); exit(0); }
	fwrite(out, sizeof(float), totalElements, fp_windowed);
	fclose(fp_windowed);
	printf("Photopeak energy-window acceptance range: %.9g .. %.9g\n",
		minimum_acceptance, maximum_acceptance);
 
	delete[] out;
 
	cout << "########################" << endl;
	cout << "Photon Electric Sysmat Written." << endl;
	cout << "Energy-windowed Photon Electric Sysmat Written." << endl;
	cout << "########################" << endl;
	return 0;
}
