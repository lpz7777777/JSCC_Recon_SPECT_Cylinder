...
调度器 `squeue --start` 当前估计首个短程于北京时间 2026-09-27 08:50 启动；
这是动态排队估计，不是完成时间承诺。
...
分区定量指标。
本地已从 q16 混合源的 1e9 XCAT 真值生成
`generated/Truth_XCAT_1e10.npz`：两路发射光子密度精确×10，器官掩膜原样保留，
旁置 JSON 记录源文件 SHA256 与线性比例；生成文件属于忽略 Git 的实验数据。
