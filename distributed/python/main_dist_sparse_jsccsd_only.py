...
from distributed.python.recon_osem_dist_sparse_jsccsd_only import run_recon_osem_dist_sparse_jsccsd_only


def parse_args():
    parser = argparse.ArgumentParser(description="Distributed sparse JSCCSD-only reconstruction for plane geometry.")
    ...
    parser.add_argument("--data-file-name", type=str, default="HotRodPhantom_7_22_30", help="Dataset name.")
    parser.add_argument("--count-level", type=str, default="5e9", help="Count level suffix.")
    ...
    parser.add_argument("--output-root", type=str, default="./Results/Reconstruction/Figure_Dist_JSCCSD", help="Output root directory.")
...
def main():
    ...
    try:
        if global_rank == 0:
            ...
            log_filename = repo_root / f"print_log_dist_sparse_jsccsd_only_{rand_suffix}.txt"
            logfile = open(log_filename, "w", encoding="utf-8")
            sys.stdout = Tee(sys.__stdout__, logfile)
            print(f"Distributed sparse JSCCSD-only chain initialized: World Size {world_size}")
            print(f"Args: {args}")
        ...
        if sensi_d_all and not args.recompute_sparse_sensi_d:
            if global_rank == 0:
                print("Distributed sparse JSCCSD-only chain uses file-defined Sensi_d.")
            s_map_arg.d = (sum(sensi_d_all) * args.s_map_d_ratio).to(device)
        else:
            if global_rank == 0:
                if sensi_d_all:
                    print("Distributed sparse JSCCSD-only chain recomputes Sensi_d from sparse Compton operators.")
                else:
                    print("Distributed sparse JSCCSD-only chain has no file-defined Sensi_d and recomputes it from sparse Compton operators.")
        ...
        run_recon_osem_dist_sparse_jsccsd_only(
            sysmat_local_all,
            sysmat_full_all,
            rotmat_all,
            rotmat_inv_all,
