...
def run_recon_osem_dist_cntstat(sysmat_l_all, rotmat_all, rotmat_inv_all, proj_l_all, iter_arg, s_map_arg, save_path):
    ...
    sysmat_all_device = [sysmat.to(device, non_blocking=True) for sysmat in sysmat_l_all]
    proj_all_device = [proj.to(device, non_blocking=True) for proj in proj_l_all]
