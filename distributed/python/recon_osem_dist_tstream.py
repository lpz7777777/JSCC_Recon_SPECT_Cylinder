...
def load_t_shard(record, device):
    rows = int(record["rows"])
    cols = int(record["cols"])
    shard_np = np.fromfile(record["path"], dtype=np.float32)
    if shard_np.size != rows * cols:
        raise ValueError(
            f"Shard size mismatch in {record['path']}: expected {rows * cols}, got {shard_np.size}."
        )

    shard_cpu = torch.from_numpy(shard_np.reshape(rows, cols))
    if torch.cuda.is_available():
        shard_cpu = shard_cpu.pin_memory()
    return shard_cpu.to(device, non_blocking=True)
...
def osem_list_mode_dist_stream(t_subset_plan_all, rotmat_all, rotmat_inv_all, img, s_map, device):
    rotate_num = rotmat_all[0].size(1)

    for subset_plan in t_subset_plan_all:
        weight_local = torch.zeros_like(img)
        ...
        for rotate_idx in range(rotate_num):
            for energy_idx, (rotmat, rotmat_inv) in enumerate(zip(rotmat_all, rotmat_inv_all)):
                ...
                for record in subset_plan[energy_idx][rotate_idx]:
                    t_block = load_t_shard(record, device)
...
def osem_joint_mode_dist_stream(
    sysmat_list_all,
    proj_list_all,
    t_subset_plan_all,
    rotmat_all,
    ...
    for subset_idx, subset_plan in enumerate(t_subset_plan_all):
        weight_local = torch.zeros_like(img)
...
def run_recon_osem_dist_tstream(
    sysmat_l_all,
    rotmat_all,
    rotmat_inv_all,
    proj_l_all,
    ...
    if global_rank == 0:
        print("\n" + "=" * 50)
        print("Starting distributed reconstruction with streamed t shards")
        print("=" * 50)
    ...
    for iter_idx in range(iter_arg.jsccd):
        img_jsccd = osem_list_mode_dist_stream(t_subset_plan_all, rotmat_all, rotmat_inv_all, img_jsccd, s_map_arg.d, device)
        if (iter_idx + 1) % iter_arg.save_iter_step == 0:
    ...
    for iter_idx in range(iter_arg.jsccsd):
        img_jsccsd = osem_joint_mode_dist_stream(
            sysmat_list_all,
            proj_list_all,
            t_subset_plan_all,
            rotmat_all,
