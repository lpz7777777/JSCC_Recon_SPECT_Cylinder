...
def run_recon_osem_dist_sparse_jsccsd_only(
    sysmat_l_all,
    sysmat_full_all,
    rotmat_all,
    rotmat_inv_all,
    ...
    del t_local_all
    dist.barrier()
    log_gpu_memory_usage("pre-iter-sparse-jsccsd-only", device)
    log_cpu_memory_usage("pre-iter-sparse-jsccsd-only")

    if global_rank == 0:
        print("\n" + "=" * 50)
        print("Starting Distributed Sparse JSCCSD-Only Reconstruction")
        print("=" * 50)
    ...
    for iter_idx in range(iter_arg.jsccsd):
        ...
        img_jsccsd = osem_joint_mode_dist_sparse(
            sysmat_list_all,
            proj_list_all,
            t_list_all,
            sysmat_full_all,
        ...
        if (iter_idx + 1) % iter_arg.save_iter_step == 0:
            ...
            save_idx += 1
            log_cpu_memory_usage(f"iter-{iter_idx + 1}-sparse-jsccsd-only")
            log_gpu_memory_usage(f"iter-{iter_idx + 1}-sparse-jsccsd-only", device)
    ...
    if global_rank == 0:
        print(f"Sparse JSCCSD-only distributed reconstruction time: {time.time() - time_start:.2f}s")
