#!/bin/bash
#SBATCH -J JSCC_TOnlineCache_LPZ
#SBATCH -p gpu_5090
#SBATCH -N 2
...
find_repo_root() {
    ...
    for start_dir in "${JSCC_REPO_ROOT:-}" "${SLURM_SUBMIT_DIR:-}" "$PWD"; do
        ...
        while [[ "$current" != "/" && $depth -lt 8 ]]; do
            if [[ -f "$current/distributed/python/main_dist_tonline_cache.py" && -f "$current/distributed/python/_path_setup.py" ]]; then
                echo "$current"
                return 0
            fi
...
GPUS_PER_NODE=8
ENTRYPOINT="$REPO_ROOT/distributed/python/main_dist_tonline_cache.py"
...
echo "Starting distributed JSCC online-cache job on nodes: $SLURM_JOB_NODELIST"
