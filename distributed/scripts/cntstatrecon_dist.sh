...
find_repo_root() {
    ...
    for start_dir in "${JSCC_REPO_ROOT:-}" "${SLURM_SUBMIT_DIR:-}" "$PWD"; do
        ...
        while [[ "$current" != "/" && $depth -lt 8 ]]; do
            if [[ -f "$current/distributed/python/main_dist_cntstat.py" && -f "$current/distributed/python/_path_setup.py" ]]; then
                echo "$current"
                return 0
            fi
...
echo "Starting distributed CntStat reconstruction on nodes: $SLURM_JOB_NODELIST"
...
srun --kill-on-bad-exit=1 \
     ...
     --rdzv_backend=c10d \
     "$REPO_ROOT/distributed/python/main_dist_cntstat.py"
