...
def _compute_angle_sigma_pos_strict(
    vector01,
    vector12,
    beta,
    sigma_pos1_sq,
    ...
    # Split the first-hit position uncertainty into:
    # 1) source -> first crystal (vector01)
    # 2) first -> second crystal (vector12)
    # If sysmat already integrates the finite first-crystal size in the source
    # response, reusing the vector01 term here would double count that blur.
    grad_pos1_source_leg = -jac_unit01_unit12 / sin_beta
    grad_pos1_inter_crystal = jac_unit12_unit01 / sin_beta
    grad_pos1 = grad_pos1_inter_crystal
    if include_pos1_source_leg_sigma:
        grad_pos1 = grad_pos1 + grad_pos1_source_leg
