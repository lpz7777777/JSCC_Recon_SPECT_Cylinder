...
def main():
    ...
    p.add_argument('--results-subdir', default='ClosedLoop')
    args = p.parse_args()
    ...
    for kind in ('Noiseless', 'Poisson'):
        manifests = list((root/args.results_subdir/kind).rglob('run_manifest.json'))
        ...
        for task in manifest['tasks']:
            ...
            iterations, step = task['iterations'], task['save_iter_step']
            assert step > 0 and iterations % step == 0
            if run_shape is None: run_shape = (iterations, step)
            assert run_shape == (iterations, step), 'Mixed iteration schedules'
            frames = iterations // step
            assert x.size == len(coords) and h.size == frames*len(coords)
            h = h.reshape(frames, -1)
    ...
    if set(histories) != {(k,e) for k in ('Noiseless','Poisson') for e in (218,440)}:
        raise ValueError('Missing channel outputs')
    iterations, step = run_shape
    selected = sorted({min(iterations, max(step, round(i/step)*step))
                       for i in (50,200,500,1000,2000,5000,10000) if i <= iterations} | {iterations})
    ...
    for energy in (440, 218):
        ...
        labels = ['Truth', f'Noiseless, iter {iterations}', f'Poisson, iter {iterations}']
        ...
        fig, axs = plt.subplots(2, len(selected)+1, figsize=(3.6*(len(selected)+1), 6), layout='constrained')
        for row, kind in enumerate(('Noiseless','Poisson')):
            image(axs[row,0], tc.max(axis=1), (-150,150,-60,60), vmax, kind+'\nTruth')
            for col, iteration in enumerate(selected, 1):
                image(axs[row,col], cart(histories[kind,energy][iteration//step-1]).max(axis=1),
                      (-150,150,-60,60), vmax, f'Iteration {iteration}')
        fig.suptitle(f'{energy} keV: actual saved iterations, coronal MIP, common truth scale')
        save(fig, f'iterations_{energy}', f'{energy} keV：'+ '、'.join(map(str,selected))+' 次迭代演化')
    ...
    for (kind, energy), frames in histories.items():
        ...
        for k,x in enumerate(frames):
            summary.append(dict(kind=kind,energy=energy,iteration=step*(k+1),
            ...
            for i,rod in enumerate(rods):
                ...
                rows.append(dict(kind=kind,energy=energy,iteration=step*(k+1),rod=i+1,
    ...
    metadata=dict(iterations=iterations,save_step=step,displayed_iterations=selected,
                  crop_mm=0,filter_sigma=0,intensity_fit=False,colormap='gray_r',
...
<p>判断要点：总量恢复接近 100% 不能说明热柱已恢复。请同时查看误差图、CRC 和 CNR 随迭代的变化，区分有限迭代造成的恢复不足和噪声增长；不能仅从早期迭代推断最终成像能力。此处不预先作有效 FOV 达标结论。</p>
...
<p>下载原始表格：<a href="data/rod_metrics.csv">热柱 CRC/CNR</a> · <a href="data/global_metrics.csv">全局误差/积分</a> · <a href="data/manifest.json">参数与来源哈希</a></p>'''+sections
    html=html.replace('1000 次 MLEM，每 50 次保存',f'{iterations} 次 MLEM，每 {step} 次保存')
